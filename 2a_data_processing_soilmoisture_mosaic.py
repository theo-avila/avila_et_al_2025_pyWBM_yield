...
def singleYearUrl(year):
    ...
    while current_dt <= end_dt:
        ...
        url = (f"{base_url}/{year}/{julian_day}/"
               f"NLDAS_MOS0125_H.A{yyyymmdd}.{hour_str}.020.nc")
...
def hourlyToDailyTminTmax(current_dt, tmp_downloads_dir):
    ...
    file_path = f"{tmp_downloads_dir}/{specific_model_path}{yyyymmdd}.{hour_str_running}.{file_type}"
    ...
    if os.path.exists(file_path):
        ...
        for hour in np.arange(0, 24, 1):
            ...
            file_path_possible = f"{tmp_downloads_dir}/{specific_model_path}{yyyymmdd}.{hour_str_possible}.{file_type}"
    ...
    final_dataset.to_netcdf(f"{tmp_downloads_dir}/{specific_model_path}{yyyymmdd}.{file_type}")
    

output_dir = "/storage/home/cta5244/work/pyWBM_yield_data/NCEPNARR_NLDAS_Hist_Temp/hydro_models/MOSAIC"
