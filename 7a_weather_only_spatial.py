...
gs  = GridSpec(nrows=len(sm_labels), ncols=len(year_chunks), figure=fig, wspace=0.025, hspace=0.025)
...
for indx, label_i in enumerate(sm_labels):
    ...
    fig.suptitle('Distribution of Weather-Induced Log Yield Shocks (All Counties)', fontsize=20, y=.93)
    ...
    for year_chunk_indx, (y0, y1) in enumerate(year_chunks):
        
        ax = axes[indx, year_chunk_indx]
            
        if indx == 0:
            ax.set_title(f'{y0}-{y1-1}', fontsize=16)
...
# output string cleaning for years
yc_str = str(year_chunks)
yc_str = str(yc_str).replace(", ", "_")
for char_i in ["(", ")", "[", "]"]:
