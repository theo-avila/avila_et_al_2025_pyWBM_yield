...
years_phase_enso_csv = pd.read_csv(f"/storage/group/pches/default/users/cta5244/enso4_loca2_underlying_models/enso_JAS_years_by_model_ssp370_4tenths_thr.csv")
...
phase_colors = {'historical':'tab:blue', 'model':'gray'}
...
for indx, label_i in enumerate(sm_labels):
    ...
    years_neu = m_neu.year.tolist()  
    #
    ws_all_hist_single = ws_all_hist.reset_index('year')
    ...
    bin_historical_obs = [pos_shock_years, neu_shock_years, neg_shock_years]
    
    # scatter plot
    for phase_idx, bin_hist_i in enumerate(bin_historical_obs):
        mean_series = bin_hist_i.loc[(slice(None), slice(None), label_i)].mean()
        y = mean_series['shock']
        ax_scatter.scatter(
            phase_idx, y, s=75, alpha=.6,
            color=phase_colors['historical'],
            label='Observational Shocks' if (phase_idx == 0 and indx == 0) else None
        )
    ...
    for ssp_i in ssps[1:]:
        for model_name_i in unique_model_names:
            ...
            for file_pattern_i in all_pattern_245[:1]:
                ...
                try:
                    phase_idx = 0
                    single_model_pos_shocks = (pos_shock_years.loc[(slice(None), slice(year_min, year_max_p1-1), label_i)]['shock'])
                    kde_pos = gaussian_kde(single_model_pos_shocks.values)
                    axes[indx, 0].plot(xs, kde_pos(xs), label=lbl, lw=1, color='gray', alpha=.8)
                    mean_series = single_model_pos_shocks.mean()
                    ax_scatter.scatter(
                ...
                try:
                    phase_idx = 1 
                    single_model_neu_shocks = (neu_shock_years.loc[(slice(None), slice(year_min, year_max_p1-1), label_i)]['shock'])
                    kde_neu = gaussian_kde(single_model_neu_shocks.values)
                    axes[indx, 1].plot(xs, kde_neu(xs), label=lbl, lw=1, color='gray', alpha=.8)
                    mean_series = single_model_neu_shocks.mean()
                    ax_scatter.scatter(
                        phase_idx, mean_series, s=75, alpha=.6,
                        color=phase_colors['model']
                    )
                ...
                try:
                    phase_idx = 2
                    single_model_neg_shocks = (neg_shock_years.loc[(slice(None), slice(year_min, year_max_p1-1), label_i)]['shock'])
                    kde_neg = gaussian_kde(single_model_neg_shocks.values)
                    axes[indx, 2].plot(xs, kde_neg(xs), label=lbl, lw=1, color='gray', alpha=.8)
                    mean_series = single_model_neg_shocks.mean()
                    ax_scatter.scatter(
                        phase_idx, mean_series, s=75, alpha=.6,
                        color=phase_colors['model']
                    )
